...
while True:
    ...
    plates = []  # Stores (x1, y1, x2, y2, text)
    ...
    for result in plate_results:
        for box in result.boxes:
            ...
            if conf > 0.35:
                ...
                plate_text = plate_text[0] if plate_text else "Unknown"
                plates.append((x1, y1, x2, y2, plate_text))
                cv2.rectangle(img, (x1, y1), (x2, y2), (0, 255, 0), 5)
                putTextRect(img, f"Plate: {plate_text}", (x1, y1 - 25), scale=1, thickness=2, colorT=(255, 255, 255), colorR=(0, 255, 0), offset=5)

    for vx1, vy1, vx2, vy2, label, conf in vehicles:
        plate_found = False
        for px1, py1, px2, py2, plate_text in plates:
